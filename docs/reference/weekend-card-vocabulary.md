...
| `Did not request sharing` · `Answers disagree`                                            | chip row                          | sharing intent — declined at registration / conflicting answers | the two of the original four sharing-intent chips that were never proximity words. **Unchanged** by the 2026-08-22 ruling — the other two (`Wants to share`, `Near another family`) are struck; see §3 and the share-mark rows above                                                                                                                                                              | kindred#2250    |
...
| `Wants to share` / `Near another family` (the two proximity word chips) | **staff ruling, 2026-08-22.** Replaced by the share-mark cluster (§2): one icon per ticked checkbox instead of an OR'd word pair that collapsed the WITH-named tick and the similar-age tick into one "Wants to share" chip and gave NEAR no visual distinction from either. `Did not request sharing` and `Answers disagree` are untouched — they were never proximity words. See §2's share-anchor/share-cluster rows and `docs/plans/2026-08-22-share-icons-spec.md` (LOCAL ONLY) / the Share Icons Lab artifact for the locked design. Pinned in `FamilyCard.test.tsx`'s "the marks kindred#2072 STRUCK" |
...
- **The four sharing-intent chips — RESOLVED 2026-08-22, two struck, two icons,
  ...
  (§3), replaced by the share-mark cluster (§2); `Did not request sharing` and
  `Answers disagree` are unchanged. The "whether the cluster consolidates"
  question this bullet used to raise is answered: no — a dominance/collapsing
  rule was proposed and explicitly reversed by the owner the same day, so
  every ticked checkbox draws its own icon (§2's share-cluster row).
