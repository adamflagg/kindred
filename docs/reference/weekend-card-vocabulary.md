...
| Mark                                                                                      | Where                             | Means                          | Why it is there                                                                                                                                                                                                                                                                                                                                                                                  | Ruled           |
| ----------------------------------------------------------------------------------------- | --------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| Share anchor — `Handshake` icon, solo circle                                              | chip row, **leads the row**       | the radio (`share.preference`): open to sharing / only if mutual / will not share / not answered | always on for every household card, zero or one mark, shaded by the answer and never by placement; `no` is quiet gray, **never red** — red was rejected because it would compete with the unmet-need glyphs; `unanswered` carries no fill at all (a dotted outline only), so it never reads positive while never hiding either. Appends the free-text Shared-request note for `yes`/`maybe`. `aria-label` is `` `Share: <state>` ``, its own test-query handle, deliberately split from the tooltip's fallback wording — see `shareMarks.ts`'s header comment for why the two text maps diverge on exactly one state | 2026-08-22      |
| Share cluster — `HeartHandshake` / `UsersRound` / `Milestone` icons, flush capsule         | chip row, right after the anchor  | the checkboxes: WITH a named family (`share.wants_with_named`, the un-ORed tick), similar-age kids, NEAR another family (`share.proximity`) | zero to three marks, one per ticked box, fixed WITH → similar-age → NEAR order, **no dominance or collapsing** — a consolidation rule was proposed and explicitly reversed by the owner the same day, so every ticked box draws its own icon. Replaces the two struck proximity word chips (§3). **Circles, not the glyphs' rounded squares** — a deliberately different shape family, because who-can-bunk-together is a different KIND of signal from a housing need. Green for WITH/similar-age (a good share candidate), indigo for NEAR (a proximity ask, not a sharing one). Each icon carries its own tooltip, repeating the free-text names verbatim under every ticked mark present, not only the first | 2026-08-22      |
...
| Child under 2                                                                             | chip row, after the glyphs        | a child under 24 months at session start | **UNGRADED** — a fact about the party, not an ask a cabin can meet, so it never goes red and is not a fifth glyph. COMPUTED server-side from birthdates (`has_child_under_two`): the form-declared `has_infant` is answered only on adult sessions and is 0 across all 3,923 production family-weekend registrations. 24 months, deliberately not the bed rule's 18; false-when-unknown, the opposite polarity from keep-the-bed. Borrows `GLYPH_BASE`'s 20px frame; **pink** at the glyphs' own 500/400 step — outside their closed set because the closure is about needs, and the ruling that added the mark chose the hue with it. Tooltip `Child under 2`; when a child is also under the 18-month bed rule (`has_bed_exempt_child`, derived from the SAME `_consumes_a_bed` call that discounts `party_size`, so the sentence and the bed count cannot disagree) it extends to say they don't count toward capacity — the ruling that retired kindred#2212's inline per-child icon | 2026-08-21      |
...
| `Did not request sharing`                                                                 | chip row                          | sharing intent — resolved eligibility is `declined` | one of the original four sharing-intent chips that were never proximity words. **Unchanged** by the 2026-08-22 ruling — `Wants to share`/`Near another family` are struck by that ruling, `Answers disagree` (this row's other original member) by a separate one; see §3 and the share-mark rows above                                                                                                                                                              | kindred#2250    |

**Share-mark geometry, not table-shaped:** a solo mark (anchor, or a one-tick
cluster) is a full circle; 2+ cluster marks flush into one capsule with a 1px
overlap (`-ml-px`) so there is no visible seam. Which corner rounds is computed
from the mark's **position in the array** (`clusterCap`), never from CSS tree
position — `ui/Tooltip` nests each icon in its own trigger element, which
defeats `:only-child` / `:last-child` (the half-pill trap). Both mark families
are gated on household grain: `resolveShareAnchor`/`resolveShareCluster` return
`null`/`[]` for a person-grain (adult weekend) party, which has no share
question to ask a single guest — so that card draws neither the anchor nor a
cluster, not a dotted "unanswered" mark for a question never asked. Full
geometry, the rejected options and the measurements:
`docs/plans/2026-08-22-share-icons-spec.md` (LOCAL ONLY) and the Share Icons
Lab artifact (`LOCKED-final-picks`),
<https://claude.ai/code/artifact/539f1b00-4d2f-4f08-a211-cbf4d8b15266>.
...
| `Wants to share` / `Near another family` (the two proximity word chips) | **staff ruling, 2026-08-22.** Replaced by the share-mark cluster (§2): one icon per ticked checkbox instead of an OR'd word pair that collapsed the WITH-named tick and the similar-age tick into one "Wants to share" chip and gave NEAR no visual distinction from either. `Did not request sharing` is untouched — it was never a proximity word. `Answers disagree` is untouched by THIS ruling too, but struck by a separate one — see the row below. See §2's share-anchor/share-cluster rows and `docs/plans/2026-08-22-share-icons-spec.md` (LOCAL ONLY) / the Share Icons Lab artifact for the locked design. Pinned in `FamilyCard.test.tsx`'s "the marks kindred#2072 STRUCK" |
| `Answers disagree`                                        | **owner ruling, 2026-08-23** — "it is dead all the way down." `DeriveShareEligibility` (Go, `lodging_requests.go`) computed and stored a per-household conflict flag as a third return value, but eligibility itself never read it — only the chip → payload → service chain downstream did: the board's chip, `ShareRequestSummary.answers_conflict` in the API schema, and the roster service's read of it. That whole chain is what this PR removes front to back: the chip, `answersConflictDetail`, `ShareRequestSummary.answers_conflict`, the Go field and computation, and the `family_camp_registrations.share_answers_conflict` column. The OTHER red chip, `Did not request sharing`, is unrelated and stays — different signal, different chip, left alone pending its own ruling. Pinned in `FamilyCard.test.tsx`'s "the marks kindred#2072 STRUCK"; the sibling slot-level absence (`ConsentFlag` no longer has `conflictCount`) is pinned in `boardLayout.test.ts`'s "rolls up no ANSWER CONFLICT" |
...
- **Pending staff input**, unchanged until they weigh in: the consent
  warning, `Inactive`.
- **The four sharing-intent chips — RESOLVED 2026-08-22, two struck, two icons,
  no consolidation.** `Wants to share` and `Near another family` are struck
  (§3), replaced by the share-mark cluster (§2); `Did not request sharing` was
  unchanged. `Answers disagree`, the fourth, is now ALSO struck (§3) — a
  separate 2026-08-23 ruling, unrelated to this consolidation question. The
  "whether the cluster consolidates" question this bullet used to raise is
  answered: no — a dominance/collapsing rule was proposed and explicitly
  reversed by the owner the same day, so every ticked checkbox draws its own
  icon (§2's share-cluster row).
...
| **The Assign modal's `People` field has no column.** `lodging_write_ins` holds `occupant_name` and `note` only                       | Land the modal **without the people count**, and scope kindred#2503 to add it — that issue body now owns the field explicitly. ✅ **Spent 2026-08-22**: kindred#2503 landed the column, the numerator and the `People` field itself (§7)                                                                                                                |
...
| ~~kindred#2503~~ | **Landed 2026-08-22** — a write-in carries an optional party size (blank still means the room is taken wholesale). The numerator resolves it over the unit tree on both orchestrators, so `free_family_beds` reads a real count instead of treating every write-in as opaque. The Assign modal's `People` field (§5a) is built: optional, blank-first, and saves on Enter exactly as `Note` does. **A `<select>` since 2026-08-23** (blank, then 1–20, shared with `WriteInCard`'s pencil as `PARTY_SIZE_CHOICES`) — it replaced an `<input type="number">` that could not tell unparseable text from an untouched field, because the control sanitises what it cannot parse to `''` before React sees it. The bound is 20 rather than the cabin's own capacity: the largest unit sleeps 19, 15 of 118 units record no capacity at all, and an over-capacity count has to stay enterable because that is what the card's red figure shows |
