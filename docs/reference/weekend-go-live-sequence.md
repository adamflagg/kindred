...
Every **placement** write requires a scenario; `lodging_assignments` is permanently admin-only by locked decision; there is no promote/publish endpoint; `lodging_assignments_sync.go` never writes back. (Availability writes take an _optional_ scenario: `1500000135` deleted the dimension for #1998, and kindred#2382 gave it back to the occupancy half alone — blank is the live board, a scope in its own right. The staff↔family role half still takes none. The argument here is about _placements_, which are still draft-only.)

So the board writes **draft placements**, and go-live means _staff arrange in Kindred, then re-key into CampMinder by hand._
...
- **Do not gate drag on the fit check**, and do not bulk-confirm cabins. (The fit check is no longer dark to begin with — kindred#2526 grades every placed cabin at face value — so confirming buys nothing but a cleared work-down list you did not earn.) `is_confirmed` asserts a human physically checked the cabin; confirmation is a property walk, not an engineering task. `confirm_lodging_units.py` is dev-only and refuses a non-loopback URL — do not run it against production.
