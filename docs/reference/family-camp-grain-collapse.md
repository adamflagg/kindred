...
| C4 | `share_eligibility` | `:377-386` | derived from the collapsed set. Used to carry a sibling `share_answers_conflict` flag too — retired 2026-08-23, "dead all the way down": computed and stored, but eligibility itself never read it. The board's chip and the API/roster-service chain that DID read it downstream are what came out with it |
