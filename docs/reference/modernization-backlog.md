...
| 4 | `refactor(api): copy.replace for dataclasses.replace` | **PR open #1579** | 1/1/1/1/1 = 5 | 3 callsites (count confirmed; audit's line numbers `:23`/`:7` were import lines — real sites `impossibility.py:120`, `phase1_parse_service.py:244`, `deduplicator.py:191`). `from copy import replace` (bare-call files) / `import copy` + dropped unused `import dataclasses` (deduplicator). Added a spec-lock test for phase1 `_sanitize_requests`. `mypy --strict` recognizes `copy.replace` (Pyright editor stubs lag 3.13). |
| 5 | `refactor(api): itertools.batched for chunking` | **PR open #1580** | 3/1/1/2/1 = 8 | **Audit-count correction:** 24 sites across **11** files (audit said "12"). **22 converted**, 1 retired, 1 deferred. Line drift: `aggregate 328→326`, `batch_processor 369→367`, `validation 198→196`. Tuple-shift handled: 3 `attendee_repository.py` chunk helpers relaxed `list[int]→Sequence[int]`; `batch_processor.py` re-wrapped `[list(b) for b in batched(...)]` for its `list[list[Any]]` return; `metrics_sql_repository.py` `[*batch, max_year]` unpacks fine. **Ruff B911** requires explicit `strict=` — used `strict=False` (preserves slice's short-final-batch semantics). **Retired:** `ai_service.py:151` (see below). **Deferred:** `validation.py:196` → fold into #6 (same `asyncio.gather` region). |
...
| `itertools.batched` for `range(0, len(x), n)` chunking | `ai_service.py:151` (row #5 execution, #1580) | The loop also builds `batch_indices = list(range(i, min(i + batch_size, len(requests))))` — `i` is reused for **absolute** index math, not just the slice offset. `batched` drops `i`; reconstructing it via `enumerate` + `batch_num * batch_size` re-introduces the arithmetic the migration exists to remove → net-negative. **Lesson: a `range(0, len)` hit is a `batched` candidate only if `i` is used *solely* to slice.** |
