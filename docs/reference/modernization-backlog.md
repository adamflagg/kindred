...
|---|---|---|---|---|
| 1 | PEP 695 generic syntax on `phase3_disambiguation_service.py` | **next** | 1/1/1/1/1 = 5 | 1 file, drop `TypeVar` import + retype methods using `[T]`. Trivial; ships fast. |
| 2 | Enable ruff `PLC0415` (`import-outside-toplevel`) + clean known offender | | 1/1/1/1/1 = 5 | One-line config + a 2-line code move in `test_analyze_objective_sensitivity.py`. Re-survey first in case more offenders crept in. |
| 3 | `chore(py): drop redundant __future__ annotations import` | | 3/1/1/1/1 = 7 | Mechanical-but-verify sweep. Codemod: `sed -i '/^from __future__ import annotations$/d'` followed by `uv run ruff check --fix --select I`. **Not strictly a no-op:** PEP 649 (3.14) defers annotation evaluation; `from __future__ import annotations` (PEP 563) stores them as strings. Code that introspects `__annotations__` directly or calls `typing.get_type_hints()` may see a different shape after the removal. Pydantic v2 / FastAPI handle both transparently in spot-checks, but verify by running the full pytest suite + `mypy --strict` after the sweep. Lefthook pre-push is the verification gate. |
