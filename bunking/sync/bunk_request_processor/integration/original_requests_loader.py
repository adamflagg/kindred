...
class OriginalRequestsLoader:
    ...
    def clear_processed_flags(self, fields: list[str] | None = None, limit: int | None = None) -> int:
        """Clear processed flags to force reprocessing.

        Filters: year, processed != '', source fields, session (via person enrollment).
        Limit is applied after all filtering — clears at most `limit` matching records.

        Args:
            fields: Source fields to clear (default: ALL_PROCESSING_FIELDS)
            limit: Max records to clear (applied after all filtering)
        ...
        # Build base filter: year + processed + source fields
        field_conditions = [f"field = '{f}'" for f in fields]
        field_filter = "(" + " || ".join(field_conditions) + ")"
        filter_str = f"year = {self.year} && processed != '' && {field_filter}"

        # Determine session filter strategy
        need_python_session_filter = False
        if self.session_cm_ids:
            valid_cm_ids = self._get_valid_requester_cm_ids()
            if len(valid_cm_ids) == 0:
                ...
                return 0
            if len(valid_cm_ids) <= 50:
                # Small set: add to PocketBase filter directly
                id_conditions = [f"requester.cm_id = {cm_id}" for cm_id in valid_cm_ids]
                filter_str = f"{filter_str} && ({' || '.join(id_conditions)})"
            else:
                # Large set: filter in Python after fetch
                need_python_session_filter = True

        logger.info(f"Clearing processed flags with filter: {filter_str}")

        # Fetch records — single fetch, strategy depends on filters
        if need_python_session_filter:
            # Must fetch all to filter in Python, include expand for cm_id check
            items = self.pb.collection("original_bunk_requests").get_full_list(
                query_params={"filter": filter_str, "sort": "-updated", "expand": "requester"}
            )
            # Filter to persons in target sessions
            items = [
                r
                for r in items
            ...
            # No Python filtering needed, limit at DB level
            ...
            # No Python filtering, no limit — fetch all
            items = self.pb.collection("original_bunk_requests").get_full_list(
                query_params={"filter": filter_str, "sort": "-updated"}
            )

        # Apply limit after Python-side filtering (when applicable)
