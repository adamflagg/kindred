...
class TestMarkRemainingPbSync:
    def test_updates_pb_rows_for_pending_orphans(self) -> None:
        ...
        with patch.object(sweep_runner_module, "solver_runs", mock_runs):
            sweep_runner_module._mark_remaining(run_ids, start_idx=1, status="cancelled", pb=mock_pb)
    ...
    def test_swallows_pb_errors_so_orchestration_continues(self) -> None:
