...
class TestSolverRunnerLockGroups:
    ...
    @pytest.mark.asyncio
    async def test_respect_locks_false_clears_lock_groups(self, mock_solver_input):
        """When respect_locks=False, lock_groups_data should be cleared."""
        mock_runs: dict[str, dict[str, str]] = {}
        with (
            patch.object(sr_module, "fetch_session_data_v2", new_callable=AsyncMock) as mock_fetch,
            patch.object(sr_module, "fetch_historical_bunking", new_callable=AsyncMock) as mock_hist,
            ...
            # Lock groups fetched but then cleared
            mock_lock_groups.assert_called_once()
            assert mock_solver_input.lock_groups_data == {}
