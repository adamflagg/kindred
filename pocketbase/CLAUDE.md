...
**Collection rules must not traverse a relation through `@request.auth`** (`@request.auth.rel.x`). The admin "view as" persona (`rbac/view_as.go`) downgrades `@request.auth.*` by swapping `e.Auth` for a clone, and a traversal is resolved by a DB join that bypasses the clone. `TestBootedSchemaNoAuthRelationTraversal` enforces this in CI.

**That guard is narrower than the real limit, and the real limit is unsolved.** In PocketBase v0.40.4, a rule's `@request.auth.X` reads the clone only for a fixed system-field allowlist (`id`, `collectionId`, `collectionName`, `email`, `emailVisibility`, `verified` — `core/record_field_resolver_runner.go`'s `plainRequestAuthFields`). Every other field — including `is_admin` and `cached_permissions`, the two fields the whole view-as feature turns on — is resolved by a live SQL join back to the real `users` row by `e.Auth.Id` (`processRequestAuthField`, same file), which reads the real, persisted values and never sees the clone. This holds for a single segment (`@request.auth.is_admin`) exactly as much as for a relation traversal. **Concretely: a preview does not restrict any collection whose rule is shaped like `@request.auth.is_admin = true || @request.auth.cached_permissions ~ "X"`** — the shape every rule in `pb_migrations/1500000077_rbac_simplify_rules.js` and its siblings uses, i.e. effectively every RBAC-gated collection. The middleware still correctly downgrades every Go-code reader of `e.Auth` (`guardConfigWrite`, `sync/api.go`). Verified evidence and the four affected test scenarios: `rbac/view_as_router_test.go`'s "KNOWN GAP" cases, with the full trace in `viewAsMiddleware`'s doc comment (`rbac/view_as.go`). This needs a design decision before the feature can claim to restrict PocketBase-rule-gated data.
...
4. **WAL checkpoint required** after database modifications — this scopes to the Go sync layer (e.g. `forceWALCheckpoint()` in `family_camp_derived.go`); migrations don't need their own, since `runHistorySync` in `main.go` checkpoints on every boot after migrations apply (see `pb_migrations/*.js` in `.coderabbit.yaml`). The rationale is durability across a `docker stop`/`start`, not reader visibility. **One deliberate exception:** `sync_runs` (`sync/sync_runs.go`) — a checkpoint on every one of ~100 writes a day is not worth what it buys. PocketBase connects `synchronous=NORMAL`, so the checkpoint is the WAL's fsync point: a committed row already survives a process kill and a `docker stop`, and the checkpoint would only close a **host power loss**. That gap is self-closing — the next job's own checkpoint flushes this row seconds later in a sweep, the next hourly run within the hour, and boot checkpoints too — so only the LAST row of a chain is exposed, for at most an hour. Visibility does not need it either: PocketBase reads its own writes through its own connection. **Reopened and re-affirmed in kindred#2617**, which added the first reader (`fetch_session_scoped_sync_ends`, driving the weekend "Housing synced" line) — #2297 said to revisit when one appeared. It holds because that reader is an *HTTP client of PocketBase*, not a second process opening `data.db`; **a reader that opens the file directly would break the visibility half and is grounds to reopen it again.** The residual edge is a host power loss, which can only make a weekend's freshness understate itself until the next covering run. Nothing else in the sync layer is exempt
