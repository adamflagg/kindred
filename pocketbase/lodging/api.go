...
// jsonErrorKey is the envelope key every handler below reports a failure
// under. It is a wire contract the frontend reads, repeated once per error
// path, so it is named rather than retyped seven times.
...
func RegisterRoutes(e *core.ServeEvent) {
	...
	e.Router.POST("/api/custom/lodging/roster-export",
		sync.RequirePermission("bunking.manage", func(re *core.RequestEvent) error {
			...
			if err != nil {
				return re.JSON(rosterExportStatus(err), map[string]string{jsonErrorKey: err.Error()})
			}
