...
export function buildCampersFromData(
  ...
  for (const attendee of attendees) {
    ...
    if (assignment) {
      ...
      if (
        assignment.expand?.bunk &&
        typeof assignment.expand.bunk === "object"
      ) {
...
export function createLookupMaps(data: {
  assignments?: Array<
    BunkAssignmentsResponse<{ person?: PersonsResponse; bunk?: BunksResponse }>
  >;
  bunks?: BunksResponse[];
}) {
  const maps = {
...
export function toAppBunkRequest(request: BunkRequestsResponse): BunkRequest {
  ...
  let requestType: "bunk_with" | "not_bunk_with" | "age_preference" =
    "bunk_with";
