...
// Reverse mapping
const URL_TO_SESSION_NAME: Record<string, string> = Object.entries(
  SESSION_NAME_TO_URL,
).reduce((acc, [name, url]) => ({ ...acc, [url]: name }), {});
...
export function findSessionByUrlSegment(
  sessions: Session[],
  urlSegment: string,
): Session | null {
...
export function sortSessionsByDate<
  T extends { name: string; start_date: string },
>(sessions: T[]): T[] {
 ...
 */
export function sortSessionDataByName<T extends { session_name: string }>(
  data: T[],
): T[] {
 ...
 */
export function sortPriorSessionData<T extends { prior_session: string }>(
  data: T[],
): T[] {
  return [...data].sort((a, b) => {
