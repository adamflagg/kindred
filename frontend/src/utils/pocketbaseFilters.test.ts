...
describe("pocketbaseFilters", () => {
  describe("formatFilter", () => {
    it("should add spaces around != operator", () => {
      expect(formatFilter('session_type!="family"')).toBe(
        'session_type != "family"',
      );
    ...
    it("should handle complex filters", () => {
      const input =
        '(session_type!="family"&&session_type!="taste")&&year=2025';
      const expected =
        '(session_type != "family" && session_type != "taste") && year = 2025';
    ...
    it("should clean up multiple spaces", () => {
      expect(formatFilter("a   =   1    &&    b   =   2")).toBe(
        "a = 1 && b = 2",
      );
    });
  ...
  describe("createExclusionFilter", () => {
    it("should create filter excluding single value", () => {
      expect(createExclusionFilter("type", ["family"])).toBe(
        'type != "family"',
      );
    });

    it("should create filter excluding multiple values", () => {
      expect(createExclusionFilter("session_type", ["family", "taste"])).toBe(
        '(session_type != "family" && session_type != "taste")',
      );
    });
  ...
  describe("createInclusionFilter", () => {
    ...
    it("should create filter including multiple values", () => {
      expect(createInclusionFilter("session_type", ["main", "taste"])).toBe(
        '(session_type = "main" || session_type = "taste")',
      );
    });
