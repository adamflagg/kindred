...
export const SCALE_DEFINITIONS: Record<ScaleType, ScaleDefinition> = {
  penalty: {
    ...
    description:
      "Penalties are costs subtracted from the optimizer's score. One satisfied request ~ 800-1500 pts, so a penalty of 3000 costs about 3-4 requests worth.",
    impactExplainer: (v) => {
      const requestsEquiv = Math.round(v / 800);
      if (v < 1000)
        return `Worth ~${requestsEquiv} request${requestsEquiv !== 1 ? "s" : ""}. Optimizer may allow this violation to satisfy more requests.`;
      if (v < 3000)
        return `Worth ~${requestsEquiv} requests. Moderately enforced-violations only when clearly beneficial.`;
      if (v < 6000)
        return `Worth ~${requestsEquiv} requests. Strictly enforced-optimizer avoids unless absolutely necessary.`;
  ...
  weight: {
    ...
    levels: [
      ...
      },
      {
        threshold: 1,
        label: "Priority",
        color: "text-forest-700 dark:text-forest-300",
        bgColor: "bg-forest-100 dark:bg-forest-900/50",
      },
    ],
    description:
      "Score weights (0-2) multiply how much a factor contributes. 1.0 = normal, 0.5 = half importance, 2.0 = double.",
    impactExplainer: (v) => {
      if (v < 0.3) return "This factor has minimal influence on decisions.";
      if (v < 0.7)
        return "This factor has moderate influence, typical for supporting criteria.";
  ...
  boost: {
    ...
    description:
      'Score boosts (0-100) add bonus points to scores. Think of them as "priority bumps" for special cases.',
    impactExplainer: (v) => {
      if (v < 10) return "A small nudge that may tip close decisions.";
      if (v < 30)
        return "A noticeable advantage that helps prioritize this case.";
  ...
  threshold: {
    ...
    description:
      "Confidence thresholds (0-100%) set the bar for automatic decisions. Higher = more certain before acting.",
    impactExplainer: (v, min = 0, max = 1) => {
      const pct = Math.round(v * 100);
      const normalized = (v - min) / (max - min);
      if (normalized < 0.5)
        return `System accepts decisions when ${pct}%+ confident. Many cases will pass automatically.`;
      if (normalized < 0.75)
        return `System requires ${pct}%+ confidence. Most clear cases pass, ambiguous ones need review.`;
      if (normalized < 0.9)
        return `System requires ${pct}%+ confidence. Only high-quality matches pass automatically.`;
  ...
  multiplier: {
    ...
    description:
      "Importance multipliers (0.5-3x) scale how much weight a source or factor carries. 1x = baseline importance.",
    impactExplainer: (v) => {
      if (v < 0.8)
        return `This source carries ${Math.round(v * 100)}% of normal weight-less influential.`;
      if (v < 1.2) return `This source carries normal weight in decisions.`;
      if (v < 2)
        return `This source is ${v.toFixed(1)}x more important than baseline.`;
...
export function inferScaleType(
  ...
  if (key.includes("threshold") || (key.includes("confidence") && maxVal <= 1))
    return "threshold";
  ...
  if (
    ...
    key.includes("importance") ||
    (key.includes("weight") && maxVal <= 3)
  )
    return "multiplier";
