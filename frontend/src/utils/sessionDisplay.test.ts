...
describe("sessionDisplay utilities", () => {
  ...
  describe("getSessionDisplayName", () => {
    ...
    it("should use parent session name for AG sessions when allSessions provided", () => {
      ...
      expect(getSessionDisplayName(agSession, mockAllSessions)).toBe(
        "Session 2",
      );
    });
    ...
    it("should fallback to original name when allSessions not provided", () => {
      ...
      expect(getSessionDisplayName(agSession)).toBe(
  ...
  describe("getSessionDisplayNameFromString", () => {
    ...
    it("should transform AG sessions by type", () => {
      expect(getSessionDisplayNameFromString("Some AG Session", "ag")).toBe(
        "Some AG Session",
      );
      expect(getSessionDisplayNameFromString("AG Session 2", "ag")).toBe(
        "Session 2",
      );
    });
    ...
    it("should transform AG sessions by name pattern", () => {
      expect(
        getSessionDisplayNameFromString("All-Gender Cabin-Session 2"),
      ).toBe("Session 2");
      expect(getSessionDisplayNameFromString("Session 3 All-Gender")).toBe(
        "Session 3",
      );
    ...
    it("should return original name if no transformation needed", () => {
      ...
      expect(getSessionDisplayNameFromString("Taste of Camp")).toBe(
        "Taste of Camp",
      );
      expect(getSessionDisplayNameFromString("Family Camp 1")).toBe(
        "Family Camp 1",
      );
    });
  });
  ...
  describe("getFormattedSessionName", () => {
    ...
    it("should return original name for AG session when parent not found", () => {
      ...
      expect(getFormattedSessionName(agSession, [])).toBe(
        "All-Gender Cabin-Session 99",
      );
    });
  ...
  describe("getSessionChartLabel", () => {
    ...
    it("should return taste session name as-is", () => {
      ...
      expect(getSessionChartLabel("Taste of Camp 2", "taste")).toBe(
        "Taste of Camp 2",
      );
      expect(getSessionChartLabel("Taste of Camp 2025", "taste")).toBe(
        "Taste of Camp 2025",
      );
    });

    it("should abbreviate AG sessions and preserve grade ranges", () => {
      expect(getSessionChartLabel("All-Gender Cabin-Session 2", "ag")).toBe(
        "All-Gender 2",
      );
      expect(
        getSessionChartLabel("All-Gender Cabin-Session 2 (Grades 6-8)", "ag"),
      ).toBe("All-Gender 2 (6-8)");
      expect(
        getSessionChartLabel("All-Gender Cabin-Session 3 (Grades 3-5) 2025"),
      ).toBe("All-Gender 3 (3-5)");
    ...
    it("should truncate very long names without grade ranges", () => {
      expect(
        getSessionChartLabel(
          "Some Very Long Session Name That Goes On Forever",
        ),
  ...
  describe("getSessionChartLabel with date lookup", () => {
    it("should return taste session name as-is, ignoring date lookup", () => {
      ...
      expect(
        getSessionChartLabel("Taste of Camp 1", undefined, dateLookup),
      ).toBe("Taste of Camp 1");
      expect(
        getSessionChartLabel("Taste of Camp 2", undefined, dateLookup),
      ).toBe("Taste of Camp 2");
    });
    ...
    it("should not transform taste session names with date lookup", () => {
      ...
      expect(getSessionChartLabel("Taste of Camp", "taste", dateLookup)).toBe(
        "Taste of Camp",
      );
    });
    ...
    it("should not append date to non-Taste sessions", () => {
      ...
      expect(getSessionChartLabel("Session 2", "main", dateLookup)).toBe(
        "Session 2",
      );
      expect(getSessionChartLabel("Session 3", "main", dateLookup)).toBe(
        "Session 3",
      );
    });
