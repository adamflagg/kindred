...
export function BunkRequestProvider({
  sessionCmId,
  children,
}: BunkRequestProviderProps) {
  ...
  const bunkPersonSetCache = React.useRef<
    Map<number, { set: Set<number>; size: number }>
  >(new Map());

  const getBunkPersonSet = (
    bunkCmId: number,
  ...
  const getSatisfiedRequestInfo = (
    ...
    const topPriority = personRequests.reduce(
      (max, req) => Math.max(max, req.priority || 0),
      0,
    );
    const topPrioritySatisfied = sortedSatisfied.some(
      (req) => (req.priority || 0) === topPriority,
    );
    const priorityLevels = [
      ...new Set(sortedSatisfied.map((r) => r.priority || 0)),
    ].sort((a, b) => b - a);
    const hasLockedPriority = satisfiedRequests.some(
      (req) => req.priority_locked,
    );
