...
export interface SolverScoreResult {
  ...
  field_scores: Record<
    string,
    { total: number; satisfied: number; raw_score: number }
  >;
...
export const solverService = {
  ...
  async pollSolverStatus(
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      const response = await fetchWithAuth(
        `${SOLVER_API_URL}/solver/run/${solverRunId}`,
      );

      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Failed to get solver status: ${response.status} - ${errorText}`,
        );
      }
  ...
  async applySolverResults(
    ...
    const response = await fetchWithAuth(
      `${SOLVER_API_URL}/solver/apply/${solverRunId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  validateConstraints(constraints: Constraint[]): string[] {
    ...
    constraints.forEach((constraint, index) => {
      ...
      if (constraint.type === "keep_apart" && constraint.campers.length < 2) {
        errors.push(
          `Keep apart constraint ${index + 1} must have at least 2 campers`,
        );
      }
    });
  ...
  async validateBunking(
    ...
    try {
      const response = await fetchWithAuth(
        `${SOLVER_API_URL}/validate-bunking`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async clearScenarioAssignments(
    ...
    try {
      const response = await fetchWithAuth(
        `${SOLVER_API_URL}/scenarios/${scenarioId}/clear`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async preValidateRequests(
    ...
    try {
      const response = await fetchWithAuth(
        `${SOLVER_API_URL}/solver/pre-validate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
