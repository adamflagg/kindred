...
export type ValidSummerSessionType =
  (typeof VALID_SUMMER_SESSION_TYPES)[number];

/**
 * Check if a session type is valid for summer views
 */
export function isValidSummerSession(sessionType: string): boolean {
  return VALID_SUMMER_SESSION_TYPES.includes(
    sessionType as ValidSummerSessionType,
  );
}
