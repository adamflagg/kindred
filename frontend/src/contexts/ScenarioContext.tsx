...
import {
  useCreateScenario,
  useDeleteScenario,
} from "../hooks/useSavedScenariosMutation";
import {
  useUpdateScenario,
  useClearScenario,
} from "../hooks/useScenarioOperations";
...
export const ScenarioProvider: React.FC<ScenarioProviderProps> = ({
  ...
  const syncToLocalStorage = useEffectEvent(
    (sessionId: number | undefined, scenarioId: string | null) => {
      ...
      const scenarioBySession: Record<string, string> = stored
        ? JSON.parse(stored)
        : {};
      ...
      localStorage.setItem(
        "scenarioBySession",
        JSON.stringify(scenarioBySession),
      );
    },
  );
