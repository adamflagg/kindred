...
const LockGroupContext = createContext<LockGroupContextValue | undefined>(
  undefined,
);
...
export function useLockGroupContext() {
  ...
  if (!context) {
    throw new Error(
      "useLockGroupContext must be used within LockGroupProvider",
    );
  }
  return context;
}
...
export function LockGroupProvider({ children }: LockGroupProviderProps) {
  ...
  const [pendingDelays, setPendingDelays] = useState<Map<string, number>>(
    new Map(),
  );
  ...
  const addPendingCamper = useCallback(
    (camper: Camper) => {
      ...
      setPendingCampers((prev) => {
        ...
        setPendingDelays((prevDelays) =>
          new Map(prevDelays).set(camper.id, delay),
        );
  ...
  const getPendingAnimationDelay = useCallback(
    (camperId: string) => {
  ...
  const getCamperLockState = useCallback(
    (camperCmId: number): "none" | "pending" | "locked" => {
  ...
  const getCamperLockGroupColor = useCallback(
    (camperCmId: number) => {
