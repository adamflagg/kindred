...
function TestConsumer() {
  const { selectedSessionCmId, selectedSession, sessions, isLoading } =
    useMetricsSession();
  return (
    <div>
      ...
      <span data-testid="selectedSessionName">
        {selectedSession?.name ?? "All Sessions"}
      </span>
...
function TestSetter({ sessionCmId }: { sessionCmId: number | null }) {
  ...
  return (
    <button
      onClick={() => setSelectedSessionCmId(sessionCmId)}
...
describe("MetricsSessionContext", () => {
  describe("initial state", () => {
    it("should default to null (all sessions) when no URL param", () => {
      ...
      expect(screen.getByTestId("selectedSessionCmId")).toHaveTextContent(
        "null",
      );
      expect(screen.getByTestId("selectedSessionName")).toHaveTextContent(
        "All Sessions",
      );
    });
  ...
  describe("clearSession", () => {
    ...
    it("should remove session URL param", async () => {
      render(
        ...
        {
          wrapper: createWrapper(
            "/metrics/registration?session=1001&year=2026",
          ),
        },
      );
  ...
  describe("URL param edge cases", () => {
    ...
    it("should preserve other URL params when updating session", async () => {
      ...
      function YearParamViewer() {
        const [searchParams] = useSearchParams();
        return (
          <span data-testid="url-year">
            {searchParams.get("year") ?? "none"}
          </span>
        );
      }
  ...
  describe("useMetricsSession hook", () => {
    it("should throw when used outside provider", () => {
      ...
      expect(() => {
        ...
        "useMetricsSession must be used within a MetricsSessionProvider",
      );
