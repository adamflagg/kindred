...
export function useCamperEnrollment(
  personCmId: string | number | null | undefined,
): UseCamperEnrollmentResult {
  const currentYear = useYear();
  const numericId = personCmId
    ? typeof personCmId === "string"
      ? parseInt(personCmId, 10)
      : personCmId
    : null;
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const sortedAttendees = [...attendees].sort((a, b) => {
        const aType =
          (a.expand as { session?: { session_type?: string } })?.session
            ?.session_type || "unknown";
        const bType =
          (b.expand as { session?: { session_type?: string } })?.session
            ?.session_type || "unknown";
      ...
      const session =
        (attendee.expand as { session?: unknown })?.session ?? null;
