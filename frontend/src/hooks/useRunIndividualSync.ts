  ...
  return useMutation({
    mutationFn: async (syncType: string) => {
      ...
      const response = await pb.send(
        `/api/custom/sync/${toEndpoint(syncType)}`,
        {
          method: "POST",
        },
      );
    ...
    onSuccess: (data, syncType) => {
      ...
      if (data?.status === "queued") {
        toast(
          `${displayName} sync queued (position ${data?.position || "?"})`,
          {
            icon: "📋",
            duration: 4000,
        ...
        toast(
          `${displayName} sync is starting - this may take a few moments due to API rate limits`,
          {
            icon: "⏳",
            duration: 6000,
            className: "toast-lodge toast-lodge-info",
