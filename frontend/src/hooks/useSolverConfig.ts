  ...
  return useQuery<SolverConfigData>({
    ...
    queryFn: async () => {
      ...
      const configs = await pb
        .collection<ConfigWithMetadata>("config")
        .getFullList({
      ...
      try {
        sections = await pb
          .collection<ConfigSectionsRecord>("config_sections")
          .getFullList({
            sort: "display_order",
          });
      } catch {
        ...
        console.warn(
          "config_sections table not found, continuing without sections",
        );
      }
...
export function useSolverConfigValue<T = unknown>(
  key: string,
  defaultValue?: T,
): T | undefined {
