  ...
  return useMutation({
    mutationFn: async (params: CreateScenarioParams) => {
      ...
      if (params.copyOptions) {
        if (
          "fromProduction" in params.copyOptions &&
          params.copyOptions.fromProduction
        ) {
          // Copy from production data
          await copyProductionToScenario(
            params.session_cm_id,
            scenario.id,
            params.year,
          );
        } else if ("fromScenario" in params.copyOptions) {
          // Copy from another scenario
          await copyScenarioToScenario(
            params.copyOptions.fromScenario,
            scenario.id,
            params.year,
          );
        }
...
async function copyProductionToScenario(
  sessionCmId: number,
  scenarioId: string,
  year: number,
) {
  // Get production assignments with expanded relations to get CM IDs
  const productionAssignments = await pb
  ...
  console.log(
    `Copying ${filteredAssignments.length} assignments to scenario ${scenarioId}`,
  );
  ...
  if (errors.length > 0) {
    console.error(
      `Failed to copy ${errors.length}/${filteredAssignments.length} assignments`,
    );
    throw new Error(
      `Failed to copy ${errors.length} assignments. Check console for details.`,
    );
  }
}

// Helper function to copy assignments from one scenario to another
async function copyScenarioToScenario(
...
export function useDeleteScenario() {
  ...
  return useMutation({
    mutationFn: async (scenarioId: string) => {
      ...
      const draftAssignments = await pb
        .collection("bunk_assignments_draft")
        .getFullList({
