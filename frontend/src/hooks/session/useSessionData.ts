 ...
 */
export function extractBunkIds(bunkPlans: Array<{ bunk: string | null }>): string[] {
 ...
 */
export function filterAgBunks<T extends { name: string }>(bunks: T[], includeAg: boolean): T[] {
  return bunks.filter((b) => (includeAg ? b.name.startsWith('AG-') : !b.name.startsWith('AG-')))
}

/**
 * Deduplicate bunks by name, keeping the first occurrence
 */
export function deduplicateBunksByName<T extends { name: string }>(bunks: T[]): T[] {
...
async function fetchBunksForSession(sessionCmId: number, currentYear: number): Promise<Bunk[]> {
  ...
  if (bunkIds.length <= 50) {
    ...
    const allBunks = await pb.collection<Bunk>('bunks').getFullList({ sort: 'name' })
...
export function useSessionBunks({
  ...
  return useQuery({
    queryKey: ['bunks', selectedSession, sessionCmId, agSessions.map((s) => s.id).sort()],
...
export function useSessionCampers({
  ...
  return useQuery({
    queryKey: ['campers', selectedSession, agSessions.map((s) => s.id).sort(), scenarioId],
    queryFn: async (): Promise<Camper[]> => {
      if (!selectedSession) return []

      const getCampersForSession = async (sessionCmId: string | number): Promise<Camper[]> => {
        try {
          const sessionResp = await pb.collection<Session>('camp_sessions').getList(1, 1, {
            filter: `cm_id = ${sessionCmId} && year = ${currentYear}`,
          })
