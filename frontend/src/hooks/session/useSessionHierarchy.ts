...
export function getAgSessions(
  ...
  // Try parent_id + session_type first (preferred)
  let ag = allSessions.filter(
    (s) => s.parent_id === parentSession.cm_id && s.session_type === "ag",
  );
  ...
  if (ag.length === 0) {
    ...
    ag = allSessions.filter(
      (s) =>
        (s.name.toLowerCase().includes("all-gender") ||
          s.name.toLowerCase().includes("ag")) &&
        (s.name.includes(sessionName) ||
          s.name.match(
            new RegExp(`\\b${sessionName.replace("Session ", "")}\\b`),
          )),
    );
  }
 ...
 */
export function shouldShowAgArea(
  agSessions: Session[],
...
export function useSessionHierarchy(
  options: UseSessionHierarchyOptions,
  ...
  const [prevResolvedSessionId, setPrevResolvedSessionId] = useState<
    number | null
  >(null);

  // Fetch all sessions to resolve friendly URLs
  const { data: allSessionsForLookup = [], isLoading: sessionsLoading } =
    useQuery({
  ...
  const resolvedSession =
    sessionId && allSessionsForLookup.length > 0
  ...
  const { data: sessionBunkPlanCounts = {}, isSuccess: bunkPlanCountsLoaded } =
    useQuery({
      queryKey: [
        "session-bunk-plan-counts",
        currentYear,
        resolvedSession?.cm_id,
      ],
      queryFn: async () => {
        ...
        const childSessions = allSessions.filter(
          (s) => s.parent_id === resolvedSession.cm_id,
        );
  ...
  useEffect(() => {
    if (
      ...
      session &&
      sessionId &&
      isNumericSessionId(sessionId)
    ) {
      const friendlyUrl = sessionNameToUrl(session.name);
      navigate(
        `/summer/session/${friendlyUrl}${tabPath ? `/${tabPath}` : "/board"}`,
        { replace: true },
      );
    }
