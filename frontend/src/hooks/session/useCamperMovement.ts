...
async function traditionalAssignment(
  ...
  const attendeeResp = await pb.collection<AttendeesResponse>('attendees').getList(1, 1, { filter })

  if (attendeeResp.items.length === 0) {
    throw new Error(`Attendee not found for person ${personCmId} in session ${sessionCmId}`)
  }
  ...
  const sessionCmIdParsed =
    typeof selectedSession === 'string' ? parseInt(selectedSession, 10) : selectedSession
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (!response) {
        ...
        // #1041 — satisfaction endpoint must refresh after camper is moved.
