...
export function useCamperEnrollment(
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const person = (attendees[0]?.expand as any)?.person as
        | PersonsResponse
        | undefined;
      if (!person) {
      ...
      const campers = attendees.map((attendee) => {
        ...
        const displayName =
          `${expandedPerson.first_name} ${expandedPerson.last_name}`.trim() ||
          "";
        ...
        return {
          ...
          age:
            expandedPerson.age ??
            (expandedPerson.birthdate
              ? calculateAge(expandedPerson.birthdate)
              : 0),
