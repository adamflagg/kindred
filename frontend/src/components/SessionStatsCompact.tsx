...
export default function SessionStatsCompact({
  ...
  const filteredBunks = bunks.filter((bunk) => {
    ...
    if (selectedArea === "boys")
      return bunkGender === "m" || bunkGender === "boys";
    if (selectedArea === "girls")
      return bunkGender === "f" || bunkGender === "girls";
  ...
  const assignedCampers = filteredCampers.filter(
    (c) => c.assigned_bunk && filteredBunkIds.has(c.assigned_bunk),
  );
  ...
  const sexBreakdown = filteredCampers.reduce(
    (acc, camper) => {
  ...
  const utilization =
    totalCapacity > 0 ? (assignedCampers.length / totalCapacity) * 100 : 0;
    ...
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-muted-foreground">
          ({assignedCampers.length} assigned)
        </span>
      </div>
