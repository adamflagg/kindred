...
export default function BunkSocialGraphModal({
  ...
  const { data: allBunks } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (sessionResp.items.length === 0) {
        throw new Error(
          `Session with CampMinder ID ${sessionCmId} not found for year ${year}`,
        );
      }
      ...
      const bunks = await pb
        .collection<Bunk>("bunks")
        .getFullList({ filter: bunkFilter });
  ...
  useEffect(() => {
    if (allBunks && allBunks.length > 0 && bunkCmId) {
      ...
      const extractSortKey = (
        name: string,
      ): { primary: number; secondary: string } => {
  ...
  const handlePreviousBunk = () => {
    if (sessionBunks.length > 0) {
      ...
      const prevIndex =
        currentBunkIndex === 0 ? sessionBunks.length - 1 : currentBunkIndex - 1;
  ...
  const handleNextBunk = () => {
    if (sessionBunks.length > 0) {
      ...
      const nextIndex =
        currentBunkIndex === sessionBunks.length - 1 ? 0 : currentBunkIndex + 1;
  ...
  return (
    ...
    >
      <Activity mode={isOpen ? "visible" : "hidden"}>
        <div
          className={`bg-card rounded-none sm:rounded-2xl shadow-lodge-xl overflow-hidden transition-all duration-300 sm:ml-4 ${
            selectedCamperId
