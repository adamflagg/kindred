...
export default function FriendGroups({
  ...
  const friendGroups = useMemo(() => {
    ...
    pairConstraints.forEach((constraint) => {
      ...
      if (missingCampers.length > 0 && constraint.campers) {
        ...
        const presentCampers = constraint.campers.filter((id) =>
          sessionCamperIds.has(id),
        );
        if (presentCampers.length > 0) {
          ...
          groupErrors.set(
            errorGroupId,
            `Request includes ${missingCampers.length} camper(s) from other sessions`,
          );
        }
        return;
      }
    ...
    groups.forEach((camperIds, groupId) => {
      ...
      const bunks = new Set<string | null>(
        groupCampers.map((c) => c.assigned_bunk || null),
      );
  ...
  const stats = useMemo(() => {
    ...
    const campersInGroups = friendGroups
      .filter((g) => g.size > 1)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(["all", "complete", "split", "isolated"] as const).map(
          (filterType) => (
            <button
              key={filterType}
        ...
        {filteredGroups.map((group) => (
          ...
          >
            ...
            <div className="space-y-2">
              {group.campers
                .slice(0, selectedGroup === group.id ? undefined : 3)
                .map((camper) => (
                  <div
                    key={camper.id}
                  ...
                  >
                    ...
                    <span className="text-muted-foreground">
                      {camper.assigned_bunk
                        ? bunkIdToName.get(camper.assigned_bunk) ||
                          camper.assigned_bunk
                        : "Unassigned"}
                    </span>
                  </div>
