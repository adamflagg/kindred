 ...
 */
function sortSessionsChronologically<T extends CampSessionsResponse>(
  sessions: T[],
): T[] {
...
function sessionsOverlap(
  main: CampSessionsResponse,
  embedded: CampSessionsResponse,
): boolean {
...
function groupSessionsByStatus(sessions: SessionWithStats[]): {
  ...
  const embeddedSessions = sessions.filter(
    (s) => s.session_type === "embedded",
  );

  const createGroups = (
    mains: SessionWithStats[],
      ...
      <div className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3
              className={`font-display font-semibold truncate group-hover:text-primary transition-colors ${
          ...
          <>
            ...
            <div className="hidden sm:flex items-center justify-end gap-3 text-sm w-[200px] flex-shrink-0">
              ...
              <div className="flex items-center gap-1 text-xs">
                ...
                <svg
                  className="h-3.5 w-3.5 text-pink-600 dark:text-pink-400 flex-shrink-0 ml-0.5"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <circle cx="12" cy="4" r="3" />
...
function StatusSectionHeader({
  ...
  return (
    <div
      className={`flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 ${config.bg} border-b ${config.border}`}
    >
      <div className={`p-1 sm:p-1.5 rounded-lg ${config.iconBg}`}>
        <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${config.text}`} />
      </div>
      <div className="flex-1 min-w-0">
        <span className={`font-semibold text-xs sm:text-sm ${config.text}`}>
          {config.label}
        </span>
      ...
      <span
        className={`text-xs sm:text-sm font-semibold ${config.text} tabular-nums`}
      >
        {count}
      </span>
    </div>
...
export default function SessionList() {
    ...
    queries: sessions.map((session) => ({
      ...
      queryFn: async () => {
        ...
        const attendees = await pb
          .collection<
            AttendeesResponse<{ person?: PersonsResponse }>
          >("attendees")
          .getFullList({
        ...
        const capacityConfig = await pb
          ...
          .catch(() => null);
        const bunkCapacity = capacityConfig?.value
          ? Number(capacityConfig.value)
          : 12;
        ...
        const filteredBunkPlans = bunkPlans.filter((bp) => {
          ...
          const isAgBunk = ["ag", "mixed", "all-gender", "nb"].includes(
            bunkGender,
          );
        ...
        const pendingReviewRequests = await pb
  ...
  const sessionsWithStats: SessionWithStats[] = sessions.map(
    (session, index) => ({
      ...session,
      statistics: statisticsQueries[index]?.data as
        | SessionStatistics
        | undefined,
    ...
    return (
      <div className="flex flex-col items-center justify-center card-lodge border-dashed px-8 py-16 text-center">
        ...
        <p className="text-muted-foreground max-w-sm">
          Import sessions from CampMinder to get started with bunking
          assignments.
        </p>
      </div>
    );
      ...
      <div className="bg-white dark:bg-card rounded-xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
        ...
        {hasUpcoming && (
          <div>
            <StatusSectionHeader
              status="upcoming"
              count={groupedSessions.upcoming.reduce(
                (sum, g) => sum + 1 + g.embedded.length,
                0,
              )}
              icon={Tent}
            />
        ...
        {hasInProgress && (
          <div>
            <StatusSectionHeader
              status="in-progress"
              count={groupedSessions.inProgress.reduce(
                (sum, g) => sum + 1 + g.embedded.length,
                0,
              )}
              icon={PlayCircle}
        ...
        {hasCompleted && (
          <div>
            <StatusSectionHeader
              status="completed"
              count={groupedSessions.completed.reduce(
                (sum, g) => sum + 1 + g.embedded.length,
                0,
              )}
              icon={CheckCircle2}
