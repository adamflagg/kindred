...
export default function SessionStats({ bunks, campers }: SessionStatsProps) {
  ...
  const effectiveCapacity = bunks.reduce((sum, bunk) => {
    const assignedToBunk = campers.filter(
  ...
  const utilization =
    effectiveCapacity > 0
      ? (assignedCampers.length / effectiveCapacity) * 100
      : 0;
  ...
  const stats = [
    ...
    },
    {
      ...
      detail: `${
        bunks.filter((b) => {
          const occupancy = campers.filter(
            (c) => c.assigned_bunk === b.id,
          ).length;
          return occupancy > 0;
        }).length
      } occupied`,
    ...
    },
    {
      ...
      icon: TrendingUp,
      color:
        utilization >= 90 ? "red" : utilization >= 70 ? "yellow" : "purple",
    ...
    },
    {
      ...
      detail:
        unassignedCampers.length === 0 ? "All assigned!" : "Need placement",
      icon: AlertCircle,
        ...
        const progressColor =
          stat.progress !== undefined && stat.color
            ? progressColorClasses[
                stat.color as keyof typeof progressColorClasses
              ]
            : "";
