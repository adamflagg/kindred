...
describe('CamperDetailsPanel', () => {
  ...
  describe('outOfScope warning banner (Task 15)', () => {
    beforeEach(() => {
      // Mock a real person so the panel renders past the "not found" gate.
      // EMMA has cm_id=100 and id='pb-emma', which matches camperId="100".
    ...
    it('renders the warning banner when outOfScope prop is set', async () => {
      const onClearFilter = vi.fn()
      render(
        <CamperDetailsPanel
          camperId="100"
          onClose={mockOnClose}
          outOfScope={{ camperName: 'Liam Garcia', onClearFilter }}
        />
      )
      await waitFor(() => {
        expect(screen.getByText(/no longer in your filtered scope/i)).toBeInTheDocument()
      })
      expect(screen.getByText(/Liam Garcia/i)).toBeInTheDocument()
    })

    it('Clear filter button calls outOfScope.onClearFilter', async () => {
      const onClearFilter = vi.fn()
      render(
        <CamperDetailsPanel
          camperId="100"
    ...
    it('does not render the banner when outOfScope is undefined', async () => {
      render(<CamperDetailsPanel camperId="100" onClose={mockOnClose} />)
      await waitFor(() => {
        // Wait for the panel to mount past the loading state
        expect(document.querySelector('.spinner-lodge')).not.toBeInTheDocument()
      })
      expect(screen.queryByText(/no longer in your filtered scope/i)).not.toBeInTheDocument()
    })
  })
})
