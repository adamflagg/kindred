...
export default function BunkingBoardByArea(props: BunkingBoardByAreaProps) {
  ...
  const getBunksByArea = () => {
    ...
    bunks.forEach((bunk) => {
      const assignedCampers = campers.filter(
        (c) => c.assigned_bunk === bunk.id,
      );
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (targetBunkId) {
      ...
      if (targetBunk && sourceCamper) {
        ...
        const isFromAGSession =
          sourceCamper.expand?.session?.session_type === "ag";
        ...
        if (isFromAGSession) {
          ...
          if (sourceCamper.gender === "M") {
            isValidGender =
              bunkGender === "m" || targetBunk.name?.startsWith("B-");
          } else if (sourceCamper.gender === "F") {
            isValidGender =
              bunkGender === "f" || targetBunk.name?.startsWith("G-");
          }
        }
    ...
    const sourceCamper = campers.find((c) => c.id === camperId);
    const lockState = sourceCamper
      ? getCamperLockState(sourceCamper.person_cm_id)
      : "none";
    ...
    if (lockState === "locked" && sourceCamper) {
      ...
      if (group) {
        const memberCmIds = getGroupMembers(group.id);
        campersToMove = campers.filter((c) =>
          memberCmIds.includes(c.person_cm_id),
        );
  ...
  const handleGlobalClick = useCallback(
    (e: MouseEvent) => {
      ...
      const isOnPanel = target.closest(
        '[data-panel="camper-details"], [data-panel="lock-group"]',
      );
      ...
      {selectedBunkForGraph && (
        <Suspense fallback={null}>
          <BunkSocialGraphModal
            ...
            onBunkChange={(cmId, name) =>
              setSelectedBunkForGraph({ cmId, name })
            }
          />
        </Suspense>
      )}
