  ...
  return (
    <div className={clsx('fixed left-4 z-40 transition-all duration-200', actionBarHeight)}>
      ...
      <button
        ...
        className={clsx(
          'shadow-lodge-lg flex items-center gap-2 rounded-xl px-4 py-2.5 transition-all',
          'hover:shadow-lodge-xl hover:scale-[1.02] active:scale-[0.98]',
          hasGroups || hasPending
            ? 'bg-primary text-primary-foreground'
            : 'bg-muted/80 text-muted-foreground hover:bg-muted border-border border backdrop-blur-sm'
        )}
      >
        ...
        {(hasGroups || hasPending) && <ChevronUp className="h-3 w-3 opacity-60" />}
      </button>

      {/* Empty State Tooltip - simple tip, no unclickable button */}
      {!hasGroups && !hasPending && showTooltip && (
        <div className="card-lodge shadow-lodge-lg animate-fade-in absolute bottom-full left-0 mb-2 w-64 p-3 text-sm">
          <p className="text-muted-foreground">
            <strong className="text-foreground">Ctrl+Click</strong> campers to select, then create a
            friend group to keep them together.
          </p>
        </div>
      )}
    </div>
