...
function extractGradeRange(name: string): [number, number] {
  ...
  const rangeMatch = name.match(
    /(\d+)(?:st|nd|rd|th)?\s*[-–&]\s*(\d+)(?:st|nd|rd|th)?/,
  );
...
function BunkCard({
  ...
  const isValidDropTarget = (): boolean => {
    ...
    const isFromAGSession =
      activeDragCamper.expand?.session?.session_type === "ag";
  ...
  const { data: requestStatus = {} } =
    useBunkRequestsFromContext(camperPersonIds);
                ...
                <div className="text-muted-foreground flex-1">
                  {gradeDistribution.single ? (
                    ...
                    <>
                      <span
                        className={
                          (gradeDistribution.ratio1 ?? 0) > 67
                            ? "text-red-600 font-medium"
                            : ""
                        }
                      >
                        {formatGradeOrdinal(gradeDistribution.grade1)}:{" "}
                        {gradeDistribution.count1}
                      </span>
                      {" | "}
                      <span
                        className={
                          (gradeDistribution.ratio2 ?? 0) > 67
                            ? "text-red-600 font-medium"
                            : ""
                        }
                      >
                        {formatGradeOrdinal(gradeDistribution.grade2)}:{" "}
                        {gradeDistribution.count2}
                      </span>
                    </>
            ...
            {ageRange &&
              (() => {
                ...
                return (
                  ...
                  >
                    Ages:{" "}
                    {(getDisplayAgeForYear(youngest, viewingYear) ?? 0).toFixed(
                      2,
                    )}{" "}
                    -{" "}
                    {(getDisplayAgeForYear(oldest, viewingYear) ?? 0).toFixed(
                      2,
                    )}
                    {ageGapWarning && " ⚠️"}
                  </div>
          ...
          {sortedCampers.length === 0 ? (
            ...
            sortedCampers.map((camper) => (
              <CamperCard
                ...
                lockState={
                  isDraftMode ? getCamperLockState(camper.person_cm_id) : "none"
                }
                lockGroupColor={
                  isDraftMode
