...
function StatCard({
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl p-3 ${highlight ? 'bg-green-50 ring-1 ring-green-200 dark:bg-green-950/30 dark:ring-green-800' : 'bg-muted/50'} `}
    >
      <div
        className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg ${highlight ? 'bg-green-100 dark:bg-green-900/50' : 'bg-background'} `}
      >
        <Icon
...
export default function SolverProgressModal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      {/* Modal */}
      <div
        className={`bg-card border-border animate-in fade-in slide-in-from-bottom-4 relative mx-4 w-full max-w-md overflow-hidden rounded-2xl border-2 shadow-2xl duration-300`}
      >
        {/* Ambient glow background */}
        <div
          className={`absolute inset-0 bg-gradient-to-b ${config.bgGlow} pointer-events-none to-transparent opacity-50`}
        />
        ...
        <div className="relative flex items-start justify-between px-6 pt-5 pb-4">
          <div>
            <h2 className="font-display text-foreground text-xl font-bold">{config.label}</h2>
            <p className="text-muted-foreground mt-0.5 text-sm">{config.sublabel}</p>
            {scenarioName && (
              <p className="text-primary mt-1 text-xs font-medium">Scenario: {scenarioName}</p>
            )}
          </div>
          <div className="flex items-center gap-1">
            {onMinimize && isInProgress && (
              <button
                onClick={onMinimize}
                className="hover:bg-muted/80 rounded-lg p-1.5 transition-colors"
                aria-label="Minimize"
              >
        ...
        <div className="relative px-6 pb-6">
            ...
            <div className="flex flex-col items-center py-6">
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div
                    className={`flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/50 dark:to-orange-900/50 ${phase === 'optimizing' ? 'animate-pulse' : ''} `}
                  >
          ...
          {isComplete && stats && (
            <div className="space-y-4 py-2">
              ...
              <div className="flex items-center justify-center gap-3 py-4">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-green-100 to-emerald-100 ring-4 ring-green-200 dark:from-green-900/50 dark:to-emerald-900/50 dark:ring-green-800">
              ...
              {stats.request_validation && stats.request_validation.impossible_requests > 0 && (
                <div className="flex items-start gap-3 rounded-xl bg-amber-50 p-3 ring-1 ring-amber-200 dark:bg-amber-950/30 dark:ring-amber-800">
          ...
          {isFailed && (
            <div className="py-4">
              <div className="flex items-center justify-center gap-3 py-4">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-red-100 to-rose-100 ring-4 ring-red-200 dark:from-red-900/50 dark:to-rose-900/50 dark:ring-red-800">
                  <XCircle className="h-8 w-8 text-red-600 dark:text-red-400" />
                </div>
              </div>

              <div className="mt-4 rounded-xl bg-red-50 p-4 ring-1 ring-red-200 dark:bg-red-950/30 dark:ring-red-800">
          ...
          {/* Action button */}
          {!isInProgress && (
            <button onClick={onClose} className="btn-primary mt-4 w-full px-4 py-3 font-semibold">
