...
export default function OptimizeBunksButton({
  ...
  const selectedLevel = OPTIMIZATION_LEVELS.find((l) => l.id === selectedLevelId) ?? DEFAULT_LEVEL
    ...
    <div className="relative">
      ...
      >
        ...
        <button
          ...
          disabled={isDisabled}
          className={`btn-primary border-primary-foreground/20 flex items-center gap-1.5 rounded-none rounded-l-xl border-r px-3 py-2 text-sm disabled:cursor-not-allowed`}
        ...
        <button
          ...
          disabled={isDisabled}
          className={`btn-primary hover:bg-primary/80 flex items-center gap-1 rounded-none rounded-r-xl px-2 py-2 disabled:cursor-not-allowed`}
          aria-expanded={isOpen}
          aria-haspopup="listbox"
          aria-label="Select optimization level"
        >
          <LevelIcon className="hidden h-3.5 w-3.5 sm:block" />
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            className="animate-in fade-in slide-in-from-top-2 fixed z-[9999] duration-200"
            style={{
          ...
          >
            <div className="bg-card border-border overflow-hidden rounded-xl border-2 shadow-lg shadow-black/10 dark:shadow-black/30">
              {/* Header */}
              <div className="bg-muted/50 border-border border-b px-4 py-2.5">
                ...
                {OPTIMIZATION_LEVELS.map((level) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-3 py-2.5 transition-all duration-150 ${isSelected ? levelWarmth.bg + ' ' + levelWarmth.ring + ' ring-1 ring-inset' : 'hover:bg-muted/50'} `}
                    >
                      {/* Icon with warmth-colored background */}
                      <div
                        className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg ${levelWarmth.iconBg} transition-all duration-200`}
                      >
                      ...
                      <div className="flex-1 text-left">
                        <div className="flex items-center justify-between gap-2">
                          <span
                            className={`text-sm font-semibold ${isSelected ? levelWarmth.text : 'text-foreground'}`}
                          >
                            {level.name}
                          </span>
                          <span
                            className={`rounded px-1.5 py-0.5 font-mono text-xs ${isSelected ? levelWarmth.bg + ' ' + levelWarmth.text : 'bg-muted text-muted-foreground'} `}
                          >
