...
export default function UnassignedCampers({
  ...
  const sortedCampers = React.useMemo(() => {
    return [...campers].sort(
      (a, b) =>
        (getDisplayAgeForYear(a, viewingYear) ?? 0) -
        (getDisplayAgeForYear(b, viewingYear) ?? 0),
    );
  }, [campers, viewingYear]);
      ...
      {!embedded && campers.length > 0 && (
        <div className="mt-4 p-3 bg-accent/10 border border-accent/30 rounded-xl">
          <p className="text-sm text-amber-700 dark:text-accent">
            <strong>Tip:</strong> Drag campers to bunks to assign them. Use
            constraints to ensure friends bunk together!
          </p>
        </div>
      )}
    </div>
