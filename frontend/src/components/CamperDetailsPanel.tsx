...
export default function CamperDetailsPanel({
  ...
  const { data: camper, isLoading: camperLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const expandedAttendee = attendee?.expand as
        | { session?: ExpandedSession }
        | undefined;
      ...
      if (attendee?.session) {
        ...
        const expandedAssignment = assignment?.expand as
          | { bunk?: ExpandedBunk }
          | undefined;
  ...
  const { data: bunkRequests = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const requests = await pb
        ...
        .getFullList<BunkRequestsResponse>({
          filter,
      ...
      return requests.map((req) => {
        const person =
          req.requestee_id && req.requestee_id > 0
            ? personMap.get(req.requestee_id)
            : undefined;
        return {
          ...req,
        ...
        siblingPersons.map(async (siblingPerson) => {
          ...
          try {
            ...
            const sortedAttendees = attendees.sort((a, b) => {
              const aExpand = a.expand as
                | { session?: ExpandedSession }
                | undefined;
              const bExpand = b.expand as
                | { session?: ExpandedSession }
                | undefined;
  ...
  const agePreferenceRequest = bunkRequests.find(
      ...
      queryFn: async () => {
        ...
        try {
          ...
          for (const req of agePrefs) {
            const allInBunk =
              bunkToPersons.get(camper.assigned_bunk_cm_id) || [];
            // Filter out the camper to get only bunkmates
            const bunkmates = allInBunk.filter(
              (b) => b.cmId !== camper.person_cm_id,
            );
  ...
  const SectionHeader = ({
    ...
    const colorClasses = {
      forest:
        "bg-forest-50 dark:bg-forest-900/60 text-forest-700 dark:text-forest-100",
      amber:
        "bg-amber-50 dark:bg-amber-900/60 text-amber-700 dark:text-amber-100",
      pink: "bg-pink-50 dark:bg-pink-900/60 text-pink-700 dark:text-pink-100",
      stone:
        "bg-stone-100 dark:bg-stone-700/60 text-stone-700 dark:text-stone-100",
    };
      ...
      <div className="px-4 py-3 bg-forest-900/50 border-b border-forest-600/20">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
          ...
          <div className="flex items-center gap-1.5 text-forest-100">
            ...
            <span>
              {camper.years_at_camp || 0}{" "}
              {(camper.years_at_camp || 0) === 1 ? "year" : "years"}
            </span>
          </div>
            ...
            {expandedSections.requests && (
              <div className="mt-2 space-y-1">
                    ...
                    return (
                      ...
                      >
                        ...
                        <CamperLink
                          ...
                          showUnresolved={
                            !isConfirmed && !!request.requestedPersonName
                          }
                        />
                        ...
                        {showSatisfaction && (
                          ...
                          >
                            {satisfactionLoading ? (
                              ...
                              <span className="sat-icon sat-icon-unknown">
                                ?
                              </span>
                            ) : null}
                          </span>
                ...
                {agePreferenceRequest?.age_preference_target &&
                  (() => {
                    const ageSatisfaction =
                      satisfactionData[agePreferenceRequest.id];
                    const prefersOlder =
                      agePreferenceRequest.age_preference_target === "older";
                    ...
                    return (
                      <div
                        className={`text-xs text-muted-foreground flex items-center gap-2 px-2 ${hasOtherRequests ? "mt-3 pt-2 border-t border-border/50" : ""}`}
                      >
                ...
                <div className="space-y-1.5">
                  ...
                  {camper.expand?.session && (
                    <div className="relative flex items-center gap-2.5">
                      ...
                      <span className="font-bold text-forest-700 dark:text-forest-300 text-sm w-11">
                        {currentYear}
                      </span>
                      ...
                      <span
                        className={`text-xs truncate ${camper.expand?.assigned_bunk ? "text-foreground font-medium" : "text-amber-600 italic"}`}
                      >
                  ...
                  {/* Historical years */}
                  {historicalData.map(
                    (record: HistoricalRecord, idx: number) => (
                      <div
                        key={`${record.year}-${idx}`}
                      ...
                      >
                        ...
                        <span className="font-semibold text-foreground text-sm w-11">
                          {record.year}
                        </span>
                        <span className="text-xs text-muted-foreground truncate">
                        ...
                        <span className="text-xs text-muted-foreground">·</span>
                        <span
                          className={`text-xs truncate ${record.bunkName === "Unassigned" ? "text-amber-600 italic" : "text-foreground"}`}
                        >
                          {record.bunkName}
                        </span>
                      </div>
            ...
            {expandedSections.siblings && (
              <div className="mt-2 space-y-2">
                {siblings.map((sibling) => (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg ${getAvatarColor(sibling.gender)} flex items-center justify-center flex-shrink-0 shadow-sm`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5">
                        <span>
                          {formatAge(
                            getDisplayAgeForYear(sibling, currentYear) ?? 0,
                          )}
                        </span>
                        <span>•</span>
      ...
      <div className="flex flex-col h-full bg-card rounded-2xl overflow-hidden shadow-lodge-lg">
        ...
        <div className="bg-gradient-to-br from-forest-700 via-forest-800 to-forest-900 text-white p-4 flex-shrink-0">
          <div className="flex items-start gap-3">
            <div
              className={`w-12 h-12 rounded-xl ${getAvatarColor(camper.gender)} flex items-center justify-center shadow-lg ring-2 ring-white/20 flex-shrink-0`}
            >
              ...
              <div className="flex items-start justify-between">
                <h2 className="text-lg font-bold truncate">
                  ...
                  {camper.preferred_name &&
                    camper.preferred_name !== camper.first_name && (
                      <span className="text-white/90 font-normal italic">
                        {" "}
        ...
        <div className="bg-gradient-to-br from-forest-700 via-forest-800 to-forest-900 text-white flex-shrink-0">
          <div className="p-5">
            <div className="flex items-start gap-4">
              {/* Avatar */}
              <div
                className={`w-16 h-16 rounded-2xl ${getAvatarColor(camper.gender)} flex items-center justify-center shadow-lg ring-4 ring-white/20 flex-shrink-0`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2">
                  <span
                    className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getGenderBadgeClasses(
