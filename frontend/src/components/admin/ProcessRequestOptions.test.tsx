...
describe("ProcessRequestOptions", () => {
  ...
  it("has session selector with friendly name options including embedded sessions", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("option", { name: /taste of camp/i }),
      ).toBeInTheDocument();
    });
    expect(
      screen.getByRole("option", { name: /^session 2$/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /session 2a/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /session 2b/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /^session 3$/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /session 3a/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /session 4/i }),
    ).toBeInTheDocument();
  });
  ...
  it("has force reprocess checkbox with warning", () => {
    ...
    expect(
      screen.queryByText(/will clear processed flags/i),
  ...
  it("has accessible structure", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /process requests/i }),
    ).toBeInTheDocument();
  ...
  it("displays helpful description text", () => {
    ...
    expect(
      screen.getByText(/process original bunk requests/i),
    ).toBeInTheDocument();
  });
