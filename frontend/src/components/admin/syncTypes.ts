...
export function getYearSyncTypes(year: number, currentYear: number) {
  ...
  return YEAR_SYNC_TYPES.filter(
    (t) => !("currentYearOnly" in t && t.currentYearOnly),
  );
}

// Get sync types for a specific phase and year
// For historical years (year !== currentYear), excludes types with currentYearOnly flag
export function getSyncTypesByPhase(
  phase: SyncPhase,
  year: number,
  currentYear: number,
) {
  return YEAR_SYNC_TYPES.filter((t) => {
    ...
    if ("currentYearOnly" in t && t.currentYearOnly && year !== currentYear)
      return false;
    return true;
  });
}
