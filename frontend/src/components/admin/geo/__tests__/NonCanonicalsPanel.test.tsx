...
describe('NonCanonicalsPanel', () => {
  it('renders merged list sorted by count descending', () => {
    render(<NonCanonicalsPanel grouped={grouped} ungrouped={ungrouped} onResolve={vi.fn()} isOpen={true} onToggle={vi.fn()} />)
  ...
  it('shows empty state when no gaps', () => {
    render(<NonCanonicalsPanel grouped={[]} ungrouped={[]} onResolve={vi.fn()} isOpen={true} onToggle={vi.fn()} />)
