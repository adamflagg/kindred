    ...
    if (error.includes("capacity issues:")) {
      ...
      for (const part of parts) {
        const match = part.match(
          /(\w+):\s*(\d+)\s*campers?,\s*(\d+)\s*beds?\s*\((\d+)\s*OVER\)/i,
        );
...
function parseWarnings(warnings: string[]): ParsedWarning[] {
  return warnings.map((warning) => {
    ...
    const conflictMatch = warning.match(
      /(.+?) has conflicting requests for (.+?) \(both/,
    );
    ...
    const unsatMatch = warning.match(
      /(\d+) campers? ha(?:ve|s)(?: only unsatisfiable)? requests/i,
    );
    if (unsatMatch?.[1]) {
      return {
...
function getNonCapacityErrors(errors: string[]): string[] {
  return errors
    .filter(
      (e) =>
        !e.includes("capacity issues:") && !e.match(/Insufficient capacity/i),
    )
    .map((e) =>
      e.replace(/\s*\(\d+\)/g, "").replace(/camper \d+/g, "a camper"),
    );
}
...
function friendlyReason(reason: string): string {
  ...
  if (
    lowerReason.includes("not in session") ||
    lowerReason.includes("not enrolled")
  ) {
    return "Not enrolled";
  }
  if (
    lowerReason.includes("gender") ||
    lowerReason.includes("different area")
  ) {
...
export default function PreValidationResultsModal({
  ...
  const requestRate =
    statistics.total_campers > 0
      ? Math.round(
          (statistics.campers_with_requests / statistics.total_campers) * 100,
        )
      : 0;

  // Group warnings by type for cleaner display
  const conflictWarnings = parsedWarnings.filter((w) => w.type === "conflict");
  const unsatisfiableWarning = parsedWarnings.find(
    (w) => w.type === "unsatisfiable",
  );
  const otherWarnings = parsedWarnings.filter((w) => w.type === "other");

  const headerContent = (
    <div
      className={`pl-5 pr-14 py-4 flex items-center gap-3 ${
        valid
        ...
        <div className="px-5 py-4 space-y-3 max-h-72 overflow-y-auto">
          ...
          {conflictWarnings.length > 0 && (
            <div className="space-y-1.5">
              ...
              {conflictWarnings.map(
                (w, i) => w.names && <ConflictCard key={i} names={w.names} />,
              )}
            </div>
          )}
            ...
            <div className="px-5 pb-4 space-y-3 animate-fade-in">
              ...
              {capacityIssues.length > 0 && (
                <div className="text-xs text-muted-foreground p-2 rounded-lg bg-primary/5 border border-primary/10">
                  ...
                  Capacity issues must be resolved before running. Check bunk
                  counts or camper enrollment.
                </div>
              )}
            </div>
          )}
