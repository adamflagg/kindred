...
export function ParseAnalysisFilters({
  ...
  return (
    <div className="card-lodge bg-parchment-100/50 dark:bg-bark-900/30 flex flex-wrap items-center gap-4 p-4">
      {/* Filter icon */}
      <div className="text-forest-600 dark:text-forest-400 flex items-center gap-2">
      ...
      <div className="relative">
        <Search className="text-bark-400 pointer-events-none absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2" />
        <input
          type="text"
          value={searchQuery}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder="Search requester..."
          className="dark:bg-bark-800 border-bark-200 dark:border-bark-700 hover:border-forest-400 dark:hover:border-forest-600 focus:ring-forest-500/20 focus:border-forest-500 placeholder:text-bark-400 dark:placeholder:text-bark-500 w-48 rounded-xl border-2 bg-white py-2.5 pr-8 pl-9 text-sm font-medium transition-all duration-200 focus:ring-2 focus:outline-none"
        />
        {searchQuery && (
          <button
            onClick={() => onSearchChange('')}
            className="hover:bg-bark-100 dark:hover:bg-bark-700 text-bark-400 hover:text-bark-600 dark:hover:text-bark-300 absolute top-1/2 right-2 -translate-y-1/2 rounded-full p-1 transition-colors"
          >
      ...
      <div className="relative">
        ...
        <ChevronDown className="text-bark-400 pointer-events-none absolute top-1/2 right-3 h-4 w-4 -translate-y-1/2" />
      </div>
      ...
      <div className="relative">
        <select
          ...
          onChange={(e) => {
            ...
            onSourceFieldChange(value ? (value as SourceFieldType) : null)
          }}
          className="dark:bg-bark-800 border-bark-200 dark:border-bark-700 hover:border-forest-400 dark:hover:border-forest-600 focus:ring-forest-500/20 focus:border-forest-500 cursor-pointer appearance-none rounded-xl border-2 bg-white py-2.5 pr-9 pl-4 text-sm font-medium transition-all duration-200 focus:ring-2 focus:outline-none"
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={onClearAll}
          disabled={isClearing}
          className="bg-bark-100 dark:bg-bark-800 text-bark-600 dark:text-bark-300 border-bark-200 dark:border-bark-700 inline-flex items-center gap-2 rounded-xl border-2 px-4 py-2.5 text-sm font-medium transition-all duration-200 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:border-rose-800 dark:hover:bg-rose-900/20 dark:hover:text-rose-400"
