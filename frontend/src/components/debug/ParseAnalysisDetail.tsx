...
function SourceToggle({
  ...
  return (
    <div className="bg-bark-100 dark:bg-bark-800 inline-flex rounded-lg p-0.5">
      <button
        type="button"
        onClick={() => onChange('debug')}
        disabled={!hasDebug}
        className={`inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-[10px] font-semibold transition-all ${
          viewMode === 'debug'
            ? 'bg-amber-100 text-amber-700 shadow-sm dark:bg-amber-900/60 dark:text-amber-400'
            : hasDebug
...
function FieldRow({
  ...
  const renderValidityBadge = () => {
    ...
    if (currentResult.is_valid) {
      return (
        <span className="text-forest-600 dark:text-forest-400 inline-flex items-center gap-1">
  ...
  return (
    <div className={`card-lodge border-l-4 ${fieldColorClass} overflow-hidden`}>
      {/* Field header */}
      <div className="bg-parchment-50/50 dark:bg-bark-900/30 border-bark-100 dark:border-bark-700 flex items-center justify-between gap-3 border-b p-3">
        <div className="flex items-center gap-2">
          <span className="text-foreground text-sm font-semibold">{fieldLabel}</span>
          {renderValidityBadge()}
        </div>
        <SourceToggle
          hasDebug={hasDebug}
      ...
      {currentResult && !currentResult.is_valid && currentResult.error_message && (
        <div className="border-b border-rose-200 bg-rose-50 px-3 py-2 dark:border-rose-800 dark:bg-rose-950/30">
      ...
      <div className="grid grid-cols-1 gap-4 p-4 lg:grid-cols-2">
        ...
        <div className="space-y-2">
          <h5 className="text-muted-foreground flex items-center gap-1.5 text-xs font-semibold">
            <FileText className="h-3.5 w-3.5" />
            Original text
          </h5>
          <div className="text-foreground dark:bg-bark-800/50 rounded-lg bg-white/50 p-3 font-mono text-sm leading-relaxed whitespace-pre-wrap">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {intents.length === 0 ? (
              <div className="text-muted-foreground dark:bg-bark-800/50 rounded-lg bg-white/50 p-3 text-sm italic">
                No intents parsed
              </div>
            ) : (
...
export function ParseAnalysisDetail({
  ...
  const fieldsKey = useMemo(() => fields.map((f) => f.original_request_id).join(','), [fields])
  ...
  if (isLoading && fields.length > 0) {
    return (
      <div className="card-lodge bg-parchment-100/30 dark:bg-bark-900/20 flex h-96 items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="text-forest-600 h-8 w-8 animate-spin" />
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="font-display text-foreground text-xl font-bold">Details</h3>
            <span className="bg-bark-100 dark:bg-bark-800 text-muted-foreground rounded-md px-2 py-0.5 font-mono text-xs">
              Select a camper
            </span>
          </div>
        </div>
        ...
        <div className="card-lodge bg-parchment-100/30 dark:bg-bark-900/20 flex h-96 items-center justify-center">
          <div className="text-muted-foreground flex flex-col items-center gap-4">
            <div className="bg-bark-100 dark:bg-bark-800 flex h-16 w-16 items-center justify-center rounded-2xl">
              <User className="text-bark-400 h-8 w-8" />
            </div>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {onReparse && (
          <button
            ...
            className="btn-primary !rounded-xl disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isReparsing ? (
              <>
