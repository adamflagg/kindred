  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="relative">
          <button
            ...
            className={`dark:bg-bark-800 border-bark-200 dark:border-bark-600 hover:border-forest-300 dark:hover:border-forest-600 flex min-w-[280px] items-center gap-3 rounded-xl border-2 bg-white px-4 py-2.5 text-left transition-all duration-200 ${isDropdownOpen ? 'ring-forest-500/30 border-forest-400 dark:border-forest-500 ring-2' : ''} `}
          >
            <FileText className="text-forest-600 dark:text-forest-400 h-4 w-4 flex-shrink-0" />
            ...
            {isLoadingList ? (
              ...
              <ChevronDown
                className={`text-muted-foreground h-4 w-4 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`}
              />
            )}
          </button>
        ...
        <button
          ...
          className={`btn-primary flex items-center gap-2 !rounded-xl disabled:cursor-not-allowed disabled:opacity-50 ${saveSuccess ? '!bg-forest-600 !border-forest-600' : ''} `}
        >
      ...
      <div className="card-lodge overflow-hidden">
        {/* Status bar */}
        <div className="border-bark-200/70 dark:border-bark-700/70 bg-parchment-100/50 dark:bg-bark-900/30 flex items-center justify-between gap-4 border-b-2 px-4 py-2.5">
          <div className="flex items-center gap-3 text-sm">
            {selectedPromptMeta && (
              <>
                <span className="bg-bark-100 dark:bg-bark-700 text-bark-600 dark:text-bark-300 rounded-md px-2 py-0.5 font-mono text-xs">
        ...
        <div className="relative" style={{ height: '500px' }}>
          {isLoading ? (
            <div className="bg-parchment-100/50 dark:bg-bark-900/30 absolute inset-0 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                <Loader2 className="text-forest-600 h-8 w-8 animate-spin" />
                <span className="text-muted-foreground text-sm">Loading prompt...</span>
              </div>
            </div>
          ) : (
      ...
      {/* Query error display */}
      {queryError && (
        <div className="card-lodge !border-rose-200 bg-rose-50 p-4 dark:!border-rose-800 dark:bg-rose-950/30">
      ...
      <p className="text-muted-foreground text-center text-xs">
        Press{' '}
        <kbd className="bg-bark-100 dark:bg-bark-700 rounded px-1.5 py-0.5 font-mono">Ctrl+S</kbd>{' '}
        or <kbd className="bg-bark-100 dark:bg-bark-700 rounded px-1.5 py-0.5 font-mono">Cmd+S</kbd>{' '}
        to save
      </p>
    </div>
  )
