...
export async function authenticateBypassMode(): Promise<boolean> {
  try {
    await pb
      ...
      .authWithPassword(BYPASS_ADMIN_EMAIL, BYPASS_ADMIN_PASSWORD);
    return true;
  } catch (error) {
    console.error(
      "Bypass mode: failed to authenticate with PocketBase admin",
      error,
    );
    return false;
  }
...
export function onAuthChange(
  callback: (token: string | null, model: RecordModel | null) => void,
) {
...
export function handlePocketBaseError(error: unknown): string {
  ...
  if (
    !DISABLE_AUTH &&
    (pbError?.status === 401 || pbError?.response?.code === 401)
  ) {
